...
public class QuestionServiceImpl implements QuestionService {
    ...
    @Override
    public void list(SearchQuestionRequest request, Handler<AsyncResult<List<Question>>> resultHandler) {
        ...
        this.client.query(query, ar -> {
            if (ar.succeeded()) {
                ...
                List<Question> questions = ar.result()
                        .getResults()
                        .stream()
    ...
    public void delete(String uuid, Handler<AsyncResult<Void>> resultHandler) {
        this.client.queryWithParams(DELETE_SQL_QUERY, new JsonArray().add(uuid), ar -> {
            if (ar.succeeded()) {
    ...
    @Override
    public void create(CreateQuestionCommand command, Handler<AsyncResult<Question>> resultHandler) {
    ...
    @Override
    public void getById(String uuid, Handler<AsyncResult<Question>> resultHandler) {

        this.client.queryWithParams(GET_BY_ID_SQL_QUERY, new JsonArray().add(uuid), ar -> {
            if (ar.succeeded()) {
                if (ar.result().getNumRows() == 0) {
                    resultHandler.handle(Future.failedFuture(new ServiceException(ErrorCode.QUESTION_NOT_FOUND.getCode(), "question not found")));
                } else {
                    Question question = rowToQuestion(ar.result().getResults().get(0));
                    resultHandler.handle(Future.succeededFuture(question));
                }
            } else {
                resultHandler.handle(Future.failedFuture(ar.cause()));
    ...
    private Question rowToQuestion(JsonArray row) {
        return Question
                .create()
